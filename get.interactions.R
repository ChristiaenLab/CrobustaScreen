...
write.csv(do.call(rbind,E(mmgraph)),'STRINGdb/mmusculus.graph.csv')
write.csv(do.call(rbind,E(hsgraph)),'STRINGdb/hsapiens.graph.csv')
...
treat <- read.csv("data/treatments.csv",row.names=1)
...
int <- int[!duplicated(int),]
write.csv(int,'data/interactions.csv')
