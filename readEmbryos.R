...
meta <- data.frame(condition = sub('_', '.', 
				   sub(' .*', '', 
				       sapply(meta, '[', 3))), 
		   grouping = dirs, 
		   batch = sapply(meta, '[', 2), 
		   slide = '', 
		   date = sub('.*([0-9]{2})([0-9]{2})([0-9]{4})', 
			      '\\3_\\1_\\2', sapply(meta, '[', 2)), 
		   image = paste0('image', sub('.*Image_([0-9]+)\\].*', 
					       '\\1', dirs)), 
		   half = '', 
		   file = dirs, 
		   stringsAsFactors = F)
...
sel <- grep('slide',dirs)
meta$slide[sel] <- sub('.* (slide)\\s?([0-9]+).*', 
		       '\\1\\2', dirs[sel])
...
meta$name <- make.names(paste(meta$cond, 
			      meta$id,
			      meta$date,
			      sep = '.'))

embryos <- split(meta[, c('name', 'file')], meta$grouping)
embryos <- lapply(embryos,
		  function(x) data.frame(name = make.unique(x$name),
					 file=x$file))
...
cellID <- sapply(celldat, 
		 function(x) factor(x[, 2],
				    levels=c("ATM","TVC")))
...
dir.csv(params,"embryodat", "data", row.names = F,
	append.date = F)
