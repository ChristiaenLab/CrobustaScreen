...
gene.names <- read.delim("cionaGeneIDs/gene_name.txt", row.names=1, stringsAsFactors=F)
ensembl <- read.delim('cionaGeneIDs/KH-ENS.blast',stringsAsFactors = F,header=F)
...
ensembl.gene <- ensembl.gene[!duplicated(ensembl.gene),]
write.csv(ensembl.gene,'ensembl/cint_ensembl.csv')
