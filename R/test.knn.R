...
test.knn <- function(dat, k, clust, 
		     sample = 0.9, reps = 1000, ...){
	n.train <- round(nrow(dat) * sample)
	cv <- replicate(reps, knn.err(dat, k, 
				      clust, n.train))
...
score.gene.network <- function(g, int){
	...
	recall <- length(intersect(e.gene, 
				   int)) / length(int)
	return(recall)
}
