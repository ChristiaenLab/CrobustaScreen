...
dot.stat <- function(y, dat, ...){
	...
	ggplot(dat,
		...
		#	  y = y, ...))+
	       aes(x=!!sym(names(dat)[1]),
			  y=!!sym(y), ...)) + 
	geom_point() + theme(plot.margin = margin(20))
}
...
plot.knn <- function(dat, g, alpha = 0.1){
	require(igraph)
	dists <- as_edgelist(g)
...
write.dot <- function(g, filename, ...){
	...
	#         dir.f(write_graph, 'file')(g, format = 'dot', filename = filename)
}
